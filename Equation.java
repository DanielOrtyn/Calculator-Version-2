 ...
 * 
 * Warnings: Does not have exponential form implemented for numbers
 * 
 * @author Daniel Ortyn
 * @version 0.1
 * @since Oct 29, 2017
 *
 *
 * @deprecated currently does not consider longs valid
 */
public class Equation {
	...
	public double solveEquationNoParen() {
		...
		LinkedList<MathematicalTerm> copyOfEquationList = new LinkedList<MathematicalTerm>();
		ListIterator<MathematicalTerm> equationIterator = equation.listIterator();
		while (equationIterator.hasNext()) {
			copyOfEquationList.add(equationIterator.next());
		}
		equationIterator = copyOfEquationList.listIterator();
		MathematicalTerm currentTerm = null;
		...
		// operations
		leftNumber = 0;
		rightNumber = equationIterator.next().isNumber().doubleValue();
		while (equationIterator.hasNext()) {
			currentTerm = equationIterator.next();
			// check if the currentTerm is a number
			if (currentTerm.isNumber() != null) {
		...
		return copyOfEquationList.getFirst().isNumber().doubleValue();
	}
